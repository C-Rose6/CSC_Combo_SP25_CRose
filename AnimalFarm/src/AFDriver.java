...
public class AFDriver {
	...
	private static void inputNew(ArrayList<Animal> animals) {
		...
		Animal ani;
		System.out.println("What is the animal type: Goat, Pig, or just Animal?");
		...
		if(type.equals("Pig")) {
			...
			ani = new Goat(height, weight, name);
		}
		else {
	...
	public static File createFile() {
		...
		try {
			...
			System.out.println("An error occured while crerating your file.");
			e.printStackTrace();
		}
		return aFarm;
	}
			...
			try {
				...
				while (reader.hasNextLine()) {
					...
					if (type.equals("Pig")) {
						...
						ani = new Pig(height, weight, type, color);
